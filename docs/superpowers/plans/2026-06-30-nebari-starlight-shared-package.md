...
|---|------|-------|--------------|----------|
| 1 | Plugin alone yields Nebari look | A minimal Starlight site whose only theming is `plugins:[nebari()]` builds and renders Nebari branding (colors, fonts, logo, footer) | narrated: demo build + home-page screenshot | VERIFIED 2026-06-30 @ e64af09: plugin build exit 0 + demo `astro build` exit 0; built `docs/dist/index.html` contains `alt="Nebari"`, `data-nebari-footer`, and the mapped brand CSS. Demo config has only `plugins:[nebari()]`. |
| 2 | OKLCH tokens mapped to Starlight vars, light+dark | Built CSS defines `--sl-color-accent*`/`--sl-color-gray*`/text/bg from nebari OKLCH values for both `:root` and dark | automated: build smoke test greps compiled CSS for mapped vars in both themes | VERIFIED 2026-06-30: `bun test packages/starlight/test` 11 pass/0 fail. build.test.ts asserts `--sl-color-accent:var(--nbr-primary)`, gray ramp `color-mix`, `--nbr-*` primitives, and both `data-theme` light+dark blocks in compiled CSS. |
| 3 | Dark/light toggle switches tokens | Toggling theme swaps to the dark token set (background/accent visibly change) | narrated: Playwright toggle + before/after screenshots | VERIFIED 2026-06-30: Playwright e2e "theme toggle switches data-theme to dark" PASS (light to dark transition; accent token resolves). 3/3 e2e. |
| 4 | Fonts self-hosted, no CDN | Poppins/Atkinson/Fira Code apply; built HTML/CSS reference only local `/fonts/*`, zero external font hosts | automated: build test asserts no `fonts.googleapis`/CDN refs + local woff2 present | VERIFIED 2026-06-30: build.test.ts "fonts are self-hosted" + "woff2 emitted" PASS; no googleapis/gstatic/typekit/cdn refs; font families present in compiled CSS. |
| 5 | Logo (light/dark) + favicon | Header shows correct logo per theme; favicon set | narrated: screenshots light + dark | VERIFIED 2026-06-30: built HTML has two `alt="Nebari"` logo imgs (CSS light/dark toggle on `[data-theme]`) + `rel="icon"` favicon as an inlined data-URI of the Nebari symbol (self-hosted, base-independent). |
| 6 | Branded footer | Nebari footer renders on every page | automated: built HTML contains footer marker | VERIFIED 2026-06-30: build.test.ts footer assertion PASS; `data-nebari-footer` present in `docs/dist` HTML across pages. |
| 7 | Demo builds + Pagefind search works | `astro build` exits 0; `dist/pagefind/` exists; a query returns a result | automated: build + Playwright search assertion | VERIFIED 2026-06-30: demo `astro build` exit 0, Pagefind "Indexed 2 pages", `docs/dist/pagefind/pagefind.js` present; e2e "search returns the seeded token" PASS. |
| 8 | Consumable by a separate project | A throwaway consumer (or the dashboard) installs the package and `astro build` succeeds with `nebari()` active | automated/narrated: install + build transcript | VERIFIED 2026-06-30: `docs` workspace depends on `@nebari/starlight: workspace:*`; its `astro build` succeeds with `nebari()` active (exit 0). prepack/prepublishOnly build hooks ensure published `dist`. |
| 9 | Correct under non-root `base` | Demo built with `base:'/demo-pack/'`: asset/link URLs include the base; page loads and nav works under it | automated: build with base + URL assertions; narrated load check | VERIFIED 2026-06-30: `cd docs && bun test` base-path test PASS (1/2 assertions). `--base /demo-pack` build: 12 `/demo-pack/`-prefixed refs, zero bare `/_astro/`. |
| 10 | a11y passes (axe) | Built demo home + a content page have no serious/critical axe violations | automated: Playwright + axe run | VERIFIED 2026-06-30: Playwright + axe e2e "no serious/critical a11y violations" PASS on `/` and `/sample/` (wcag2a + wcag2aa). |
| 11 | Token sync works + documented | Running `sync-tokens` pulls `@nebari/theme` tokens into the vendored CSS; README documents provenance + refresh | narrated: script run transcript + diff | VERIFIED 2026-06-30: sync-tokens.test.ts PASS (namespacing decls + var() refs + idempotency); `nebari-tokens.css` carries the provenance header; README documents `bun run sync-tokens`. |
| 12 | EffVer tag pinnable | Repo has an EffVer tag (`vMACRO.MESO.MICRO`) and a consumer can pin to it | narrated: `git tag` + consumer pin | VERIFIED 2026-06-30: `git tag v0.1.0` created on final commit; `git describe --tags` = v0.1.0; package `version` 0.1.0; consumers pin to the v0.MESO macro line. |
